            ...
            [
              m('.header-left', {style: 'display: flex; align-items: center; gap: 8px;'},
                ...
                m('button.plus-btn', {
                  ...
                  onclick: () => {
                   state.set(draft => { draft.isBrushModalOpen = true; });
                  },
                }, '+'),
...
async function sendAuthenticatedRequestWithCorpSso() {
 ...
 const data = {
   metric_id: state.brushState.metric,
   page_size: state.brushState.pageSize
  };

 state.set(draft => { draft.brushState.metricResponse = 'Fetching metric...'; });
 ...
 try {
   ...
   state.set(draft => {
     draft.brushState.metricResponse = JSON.stringify(result, null, 2);
     if (result.columnNames && result.rows) {
       draft.brushState.tableData = {
         columns: result.columnNames,
         rows: result.rows.map((row: any) => row.values)
       };
     }
   });

 } catch (e) {
   state.set(draft => { draft.brushState.metricResponse = 'Error: ' + e; });
 }
}
...
async function sendQuery() {
 ...
 const data = {
   perfetto_sql: state.brushState.perfettoSql,
   trace_address: 'android_telemetry.field_trace_summaries_prod.last30days',
   limit: state.brushState.limit
 };

 state.set(draft => { draft.brushState.queryResponse = 'Sending query...'; });
 ...
 try {
   ...
   state.set(draft => {
     draft.brushState.queryResponse = JSON.stringify(result, null, 2);
     if (result.columnNames && result.rows) {
       draft.brushState.tableData = {
         columns: result.columnNames,
         rows: result.rows.map((row: any) => row.values)
   ...
   state.set(draft => { draft.brushState.queryResponse = 'Error: ' + e; });
 }
}

const BrushModal: m.Component = {
 view: () => {
   if (!state.isBrushModalOpen) {
     return null;
   }
   ...
   return m('.modal-overlay', [
     m('.modal-content', [
       m('button.modal-close', {
         onclick: () => state.set(draft => { draft.isBrushModalOpen = false; }),
       }, '×'),
       m('h2', 'Import Data from Brush'),
       m('.brush-modal-container', [
         m('.section', [
           m('h2', 'Metrics API'),
           m('.query-controls', [
             m('label', {for: 'metric'}, 'Metric:'),
             m('select#metric', {
               style: 'width: auto; flex-grow: 1;',
               onchange: (e: Event) => state.set(draft => { draft.brushState.metric = (e.target as HTMLSelectElement).value; }),
               value: state.brushState.metric,
             }, [
               m('option', {value: 'android_anr'}, 'android_anr'),
               m('option', {value: 'android_jank'}, 'android_jank'),
               m('option', {value: 'android_startup'}, 'android_startup'),
             ]),
             m('button', {onclick: sendAuthenticatedRequestWithCorpSso}, 'Fetch Metric'),
          ]),
          m('.query-controls', [
            m('label', {for: 'page-size'}, 'Page Size:'),
            m('input[type=number]#page-size', {
              oninput: (e: Event) => state.set(draft => { draft.brushState.pageSize = parseInt((e.target as HTMLInputElement).value, 10); }),
              value: state.brushState.pageSize,
            }),
           ]),
           m('pre', state.brushState.metricResponse),
         ]),
         m('.section', [
           m('h2', 'Query API'),
           m('label', {for: 'perfetto_sql'}, 'Perfetto SQL:'),
           m('textarea#perfetto_sql', {
             oninput: (e: Event) => state.set(draft => { draft.brushState.perfettoSql = (e.target as HTMLTextAreaElement).value; }),
             value: state.brushState.perfettoSql,
           }),
           m('.query-controls', [
             m('label', {for: 'limit'}, 'Limit:'),
             m('input[type=number]#limit', {
               oninput: (e: Event) => state.set(draft => { draft.brushState.limit = parseInt((e.target as HTMLInputElement).value, 10); }),
               value: state.brushState.limit,
             }),
             m('button', {onclick: sendQuery}, 'Send Query'),
           ]),
           m('pre', state.brushState.queryResponse),
         ]),
       ]),
      m('button.next-btn', {
         onclick: () => {
           if (state.brushState.tableData) {
             const { columns, rows } = state.brushState.tableData;
             ...
             const schemaName = `Brush Import ${new Date().toLocaleTimeString()}`;
             state.set(draft => {
               draft.dataSources[schemaName] = {
                 data: data,
                 columns: columns,
               };
               draft.isBrushModalOpen = false;
               switchDataSource(schemaName);
             });
           }
         },
